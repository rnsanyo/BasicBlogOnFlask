...
# # #Inspects the "next" URL parameter
...
# # Creating load user (required in docs):
...
def login():
    ...
    if login_form.validate_on_submit():
        user_to_log = User.query.filter_by(email=request.form["email"]).first()
        if not user_to_log:
            flash("This email does not exist.")
            return redirect(url_for("login"))
        check_password = check_password_hash(user_to_log.password, request.form["password"])
        if check_password:
            login_user(user_to_log)
            return redirect(url_for("get_all_posts"))
            # # #In case of a redirection to the previous attempted page access.
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        if logged_in:
            ...
            return redirect(url_for("login"))
    return render_template("post.html", post=requested_post, form=comment_form, logged_in=logged_in, all_comments=all_comments)
